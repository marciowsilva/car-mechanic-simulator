// ui.js
import { gameState, scene3D, upgradeSystem, achievementSystem, db } from './game.js';
...
export class UIManager {
    ...
    initEventListeners() {
        ...
        document.getElementById('new-job').addEventListener('click', () => {
            if (gameState.currentJob && !document.getElementById('deliver-car').disabled) {
                if (!confirm('Há um carro pronto. Iniciar novo serviço fará você perder o pagamento. Continuar?')) {
                    return;
                }
            }
            ...
            this.showNotification(`🚗 Cliente: ${job.customerName} - Pagamento: R$ ${job.payment}`, 'success');
        ...
        // Upgrade shop button
        document.getElementById('upgrade-shop-btn').addEventListener('click', () => this.openUpgradeShop());
    }
    ...
    updateJobInfo() {
        ...
        let difficultyText = gameState.currentJob.difficulty === 'easy' ? 'Fácil' : 
                            gameState.currentJob.difficulty === 'medium' ? 'Médio' : 'Difícil';
    ...
    updatePartsList() {
        ...
        Object.entries(gameState.currentCar.parts).forEach(([partName, partData]) => {
            ...
            const conditionClass = gameState.currentCar.getPartConditionClass(condition, targetCondition);
            
            const toolStats = upgradeSystem.getToolStats(gameState.selectedTool);
            const repairEfficiency = upgradeSystem.calculateRepairEfficiency(toolStats.repair);
            const repairCost = upgradeSystem.calculateRepairCost(toolStats.cost);
            const partPrice = upgradeSystem.calculatePartPrice(partData.price);
            ...
            const progressPercent = Math.min(100, (condition / targetCondition) * 100);
    ...
    checkJobCompletion() {
        if (!gameState.currentJob || !gameState.currentCar) return;
        const completed = gameState.currentJob.checkCompletion(gameState.currentCar.parts);
    ...
    updateTimer() {
        if (gameState.currentJob && !gameState.currentJob.isExpired()) {
            ...
            if (gameState.currentJob.isExpired()) {
                this.showNotification('⏰ Tempo esgotado! O cliente foi embora.', 'error');
