...
import { GarageRadio } from "/src/systems/GarageRadio.js";
...
class GameState {
  ...
  updateMoney(amount) {
    ...
    if (moneyEl)
      moneyEl.textContent = `R$ ${this.money.toLocaleString("pt-BR")}`;
  ...
  canAfford(amount) {
    return this.money >= (Number(amount) || 0);
  }
  ...
  repairPart(partName) {
    if (!this.currentCar?.parts[partName])
    ...
    return {
      success: true,
      message: `🔧 Reparou ${Math.round(part.condition - oldCondition)}% com ${tool.name}`,
      repairedAmount: Math.round(part.condition - oldCondition),
      partName,
...
window.createRepairEffect = (partName) => {
  if (window.scene3D?.createRepairEffect)
