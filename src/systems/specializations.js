...
export const SPECIALIZATIONS = {
  ...
  transmission: {
    id: "transmission",
    name: "Transmissão",
    desc: "Reparos na transmissão +20%",
    parts: ["transmissao"],
    bonus: 0.2,
    price: 2000,
    icon: "🔄",
  ...
  exhaust: {
    id: "exhaust",
    name: "Escape",
    desc: "Reparos no escapamento +20%",
    parts: ["escapamento"],
    bonus: 0.2,
    price: 2000,
    icon: "💨",
...
export class SpecializationSystem {
  ...
  buySpecialization(specId) {
    ...
    if (!spec)
      return { success: false, message: "Especialização não encontrada" };
    ...
    Object.values(this.specializations).forEach((spec) => {
      if (spec.level > 0) {
        spec.parts.forEach((partName) => {
          this.activeBonuses[partName] =
            (this.activeBonuses[partName] || 0) + spec.bonus * spec.level;
        });
      }
    });
  }
  ...
  getStats() {
    const totalLevels = Object.values(this.specializations).reduce(
      (sum, s) => sum + s.level,
      0,
    );
    const maxPossible = Object.values(this.specializations).reduce(
      (sum, s) => sum + s.maxLevel,
      0,
    );
    const totalSpent = Object.values(this.specializations).reduce(
      (sum, s) => sum + s.price * s.level,
      0,
    );
