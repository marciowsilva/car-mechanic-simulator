...
export class Scene3D {
  ...
  setupEnvironment() {
    ...
    const floorMat = this.getCachedMaterial("standard", 0x2a2a2a, {
      roughness: 0.8,
      metalness: 0.1,
    });
    ...
    const markMat = this.getCachedMaterial("standard", 0x1a1a1a, {
      transparent: true,
      opacity: 0.3,
    });
  ...
  createWalls() {
    const wallMat = this.getCachedMaterial("standard", 0x3a3a3a, {
      roughness: 0.7,
    });
    const windowMat = this.getCachedMaterial("standard", 0x88aaff, {
      emissive: 0x112233,
    });
  ...
  createToolCabinets() {
    ...
    GARAGE_CONFIG.toolCabinets.forEach((cabinet) => {
      ...
      for (let i = 0; i < 3; i++) {
        // Reduzido de 4 para 3 gavetas
        const drawer = new THREE.Mesh(drawerGeo, drawerMat);
        drawer.position.set(
          cabinet.position[0],
          0.3 + i * 0.4,
          cabinet.position[2] + 0.35,
        );
        drawer.castShadow = true;
        this.furniture.add(drawer);
      }
  ...
  async createCar(carData, job) {
    ...
    const carGroup = await this.carModelLoader.loadModel(carType, carColor);

    // Posicionar no elevador
    if (GARAGE_CONFIG.lifts && GARAGE_CONFIG.lifts.length > 0) {
      const activeLift =
        GARAGE_CONFIG.lifts.find((l) => !l.occupied) || GARAGE_CONFIG.lifts[0];
      carGroup.position.set(
        activeLift.position[0],
        0.3,
        activeLift.position[2],
      );
  ...
  // Método para pré-carregar modelos durante o loading
  async preloadCarModels() {
    console.log("🔄 Pré-carregando modelos de carros...");

    return new Promise((resolve) => {
      this.carModelLoader.preloadAllModels((progress) => {
  ...
  updatePartLabels(carData, job) {
    ...
    Object.entries(PART_POSITIONS).forEach(([partName, pos]) => {
      if (carData.parts[partName]) {
        ...
        normalDiv.style.color =
          condition >= targetCondition * 0.7 && condition < targetCondition
            ? "black"
            : "white";
        normalDiv.style.border =
          condition === 100 ? "3px solid gold" : "2px solid #ff6b00";
  ...
  createRepairEffect(position) {
    ...
    const particleMat = this.getCachedMaterial("standard", color, {
      emissive: 0x331100,
    });
