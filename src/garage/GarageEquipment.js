...
export const GarageEquipment = {
    ...
    paintShop: {
        ...
        unlockLevel: 3,
        price: 20000,
        paintJobs: ['Vermelho', 'Azul', 'Preto', 'Branco', 'Prata', 'Fosco', 'Metálico'],
        category: 'cosmetic'
    },
    detailingToolkit: {
        id: 'detailingToolkit',
    ...
    carLift: {
        id: 'carLift',
        name: 'Guincho',
        description: 'Transporte carros comprados no leilão',
        icon: '🚛',
        position: [-12, 0, 0],
        rotation: 0,
        unlockLevel: 4,
...
export function getEquipmentByCategory(category) {
    return Object.values(GarageEquipment).filter(eq => eq.category === category);
}

export function getUnlockedEquipment(garageLevel) {
    return Object.values(GarageEquipment).filter(eq => eq.unlockLevel <= garageLevel);
}
